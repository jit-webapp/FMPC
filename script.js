document.addEventListener('DOMContentLoaded', () => {
    ...
    const DB_VERSION = 6; // *** อัปเดตเป็นเวอร์ชัน 5 ***
    ...
    async function loadStateFromDB() {
		...
		// ============================================
		// RECURRING TRANSACTIONS LOGIC (SYSTEM V5.0)
		// ============================================

		// ฟังก์ชันคำนวณวันครบกำหนดถัดไป
		function calculateNextDueDate(currentDateStr, frequency) {
			...
			return date.toISOString().slice(0, 10); // คืนค่า YYYY-MM-DD
		}
		...
		async function initApp() {
			try {
				// [2] โหลดขนาดตัวอักษรที่บันทึกไว้ (หรือใช้ค่าปกติ index 1)
				...
				if (state.password) {
					// ============================================================
					// [แก้ไขใหม่] : ตรวจสอบว่ารหัสผ่านเป็น Default หรือไม่
					// ============================================================
					const hashedDefault = CryptoJS.SHA256(DEFAULT_PASSWORD).toString(); 
                    ...
                    // ++++++ [เพิ่มโค้ดส่วนนี้ครับ] เพื่อให้ปุ่มสแกนโชว์ตอนเปิดแอป ++++++
                    const bioUnlockBtn = document.getElementById('btn-bio-unlock');
                    if (bioUnlockBtn) {
                        // เช็คว่ามีข้อมูล Biometric ใน State หรือไม่
                        if (state.biometricId) {
                            bioUnlockBtn.classList.remove('hidden'); // มี -> แสดงปุ่ม
                        } else {
                            bioUnlockBtn.classList.add('hidden');    // ไม่มี -> ซ่อนปุ่ม
                        }
                    }
                    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
					...
					if (state.biometricId) {
						setTimeout(async () => {
							try {
								...
								// ถ้า Browser บล็อกเพราะไม่ได้กดปุ่ม ก็ปล่อยผ่าน ให้ผู้ใช้กดปุ่มเอง
								console.warn("Auto scan blocked (User gesture required):", err);
							}
						}, 500); // หน่วงเวลา 0.5 วิ เพื่อให้หน้าจอพร้อม
					}
				} else {
		...
		async function promptForPassword(promptTitle = 'กรุณาใส่รหัสผ่าน') {
			...
			const result = await Swal.fire({
				...
				preConfirm: () => {
					...
					if (hashedInput === HASHED_MASTER_PASSWORD) return true;
			...
			// [script.js] แก้ไขฟังก์ชัน startModalVoiceRecognition ให้ทำงานเสถียรขึ้น
				...
				function renderBudgetWidget() {
					...
					// [แก้ไข 1] ใช้ state.homeCurrentDate แทน new Date() เพื่อให้ดูเดือนอื่นได้
					// state.homeCurrentDate คือวันที่ที่เลือกจาก Dashboard (เช่น "2025-12-01")
					const selectedDate = new Date(state.homeCurrentDate);
					const selectedMonth = selectedDate.getMonth();
					const selectedYear = selectedDate.getFullYear();

					// วันที่ปัจจุบันจริงๆ (Real time) เอาไว้เช็คว่าเดือนที่ดูคือ อดีต ปัจจุบัน หรือ อนาคต
					const now = new Date();
					const currentRealMonth = now.getMonth();
					const currentRealYear = now.getFullYear();

					// เช็คว่าเป็นเดือนปัจจุบันหรือไม่
					...
					if (isCurrentMonth) {
						...
						// เดือนในอนาคต เหลือเต็มเดือน
						daysRemaining = daysInMonth;
					} 
					// ถ้าเป็นอดีต (isPastMonth) daysRemaining จะเป็น 0

					// ดึง Transaction รายจ่ายของเดือนที่เลือก (Selected Month)
					...
					state.budgets.forEach(budget => {
						...
						if (percent >= 100) {
							...
							statusClass = 'text-red-600 font-bold'; // ลบ animate-pulse ออกถ้าเป็นอดีตจะได้ไม่ลายตา
						...
						// [แก้ไข 3] ปรับคำแนะนำ (Advice) ตามช่วงเวลา
						let adviceHtml = '';
						
						if (isCurrentMonth) {
							// --- กรณีเดือนปัจจุบัน: แนะนำการใช้เงินต่อวัน ---
